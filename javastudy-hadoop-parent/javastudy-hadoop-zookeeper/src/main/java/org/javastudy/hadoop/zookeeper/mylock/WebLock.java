package org.javastudy.hadoop.zookeeper.mylock;
...
// lock的分布式锁 可能使用,不错
public class WebLock implements Watcher {
	...
	private static final String CONNECTION_STRING = "127.0.0.1:2183,127.0.0.1:2182,127.0.0.1:2183";
	...
	private static final Logger LOG = LoggerFactory.getLogger(WebLock.class);

	public WebLock(int id) {
		this.threadId = id;
	...
	public static void main(String[] args) {
		for (int i = 0; i < THREAD_NUM; i++) {
			...
			new Thread() {
				...
				public void run() {
					try {
						WebLock dc = new WebLock(threadId);
